...
- Split run_playbook function into two seperate functions
...
def run_playbook(device_list, action_list, cb_def, args):
    ...
    for device in device_list:
        ...
        for action in action_list:
            ...
            print("\tProcessing command <{0}> for action <{1}>...".format(actionType, command))
    ...
    for f in as_completed(futures.keys(), timeout=100):
        if f.exception() is None:
            print(f"\tDevice ID {futures[f]} job completed ({f.result()})") # if f.result = None then the job is succesful
            completed_sensors.append(futures[f])
        else:
            print(f"\tDevice ID {futures[f]} had the following error: {f.exception()}")

    still_to_do = set([device.get('device_id') \
                  for device in device_list]) - set(completed_sensors)
...
def get_device_ids_from_file(device_file):
    ...
    :param object device_file: device file name passed by the -F argunemt
...
def get_offline_devices(online_device_list, device_ids):
    ...
    :parap list device_ids: User submitted device list.
